...
class C(BaseConstants):
    NAME_IN_URL = 'attainable_occupation'
    ...
    #print("ATTAINABLE")
    #print(os.path.join(parent_dir, "_static/all_embed.csv"))
...
class Player(BasePlayer):
    ...
    attainable_occupation = models.StringField(
        label='Thinking about your current skills, knowledge, and education, what is a job you would be both interested in and qualified for as your next role?'
    )
    
    selected_attainable_job = models.StringField(
        label="Select the closest match to your attainable job"
    )
    
    top_match_indices_attainable = models.StringField()
...
class AttainableOccupation(Page):
    
    form_model = 'player'
    form_fields = ['attainable_occupation']
    ...
    def is_displayed(player):
        ...
        if condition0:
            ...
            if player.participant.vars['valid_choice'] == False:
                condition2 = player.in_round(player.round_number - 1).selected_attainable_job == "None of the above are close"
    ...
    def error_message(player, values):
        ...
        if values['attainable_occupation']=='':
            return 'Please enter a valid occupation.'
    
    def before_next_page(player, timeout_happened):
        
        # Get user's input
        respondent_job_title = player.attainable_occupation
        player.top_match_indices_attainable = start_backend_process(player, respondent_job_title)

        
class AttainableOccupationSelection(Page):
    
    form_model = 'player'
    form_fields = ['selected_attainable_job']
    ...
    def is_displayed(player):
        ...
        if player.round_number == 1:
            ...
            if player.participant.vars['valid_choice'] == False:
                return player.in_round(player.round_number - 1).selected_attainable_job == "None of the above are close"
            else:
                return False
    
    def vars_for_template(player: Player):
        
        closest_indices = list(map(int, player.top_match_indices_attainable.split('+')))
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.selected_attainable_job != 'None of the above are close':
            player.participant.vars['valid_choice'] = True
        else:
            player.participant.vars['valid_choice'] = False
        
        player.participant.vars['attainable_job'] = player.selected_attainable_job
        #print(type(C.job_title_embeddings))
        
    def error_message(player, values):
        
        #print('value is not :', values['selected_job']=='')
        if values['selected_attainable_job']=='':
            return 'Please select an option.'

        
page_sequence = [AttainableOccupation, AttainableOccupationSelection]
