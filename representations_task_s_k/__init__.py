...
class C(BaseConstants):
    NAME_IN_URL = 'representations_task_2'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1
    print(os.path.dirname(os.path.abspath(__file__)))
    print(f"**************{NAME_IN_URL}*******************")
    pairs = pd.read_csv(os.path.dirname(os.path.abspath(__file__))+"/triplets_30_k_s.csv")
...
class Player(BasePlayer):
    ...
    target_job1_sk = models.StringField()
    target_job2_sk = models.StringField()
    focal_job = models.StringField()
    choice_sk = models.StringField()
    time_sk = models.StringField()
    s_count_task2 = models.IntegerField()
    wa_count_task2 = models.IntegerField()
    k_count_task2 = models.IntegerField()
...
class ChoicePage(Page):
    ...
    form_model = 'player'
    form_fields = ['choice_sk', 'time_sk', 'target_job1_sk', 'target_job2_sk']
    
    def vars_for_template(player):
        
        
        job = player.participant.vars['onet_job']
        #job = "Actors" # for testing
    ...
    def before_next_page(player, timeout_happened):
        ...
        player.participant.vars['choice_sk'] = player.choice_sk
        
        summary_counts = find_hierarchy(player.target_job1_sk, player.target_job2_sk, player.choice_sk, player.participant.vars['onet_job'], player)
        #player.participant.vars['onet_job'] = "Actors"
        player.participant.vars['s_count_task2'] = int(summary_counts['closer_on_skills'])
        player.participant.vars['k_count_task2'] = int(summary_counts['closer_on_knowledge'])
        player.participant.vars['wa_count_task2'] = int(summary_counts['closer_on_activities'])
        
        #player.onet_job = "Floral Designers"
        player.s_count_task2 = int(summary_counts['closer_on_skills'])
        player.k_count_task2 = int(summary_counts['closer_on_knowledge'])
        player.wa_count_task2 = int(summary_counts['closer_on_activities'])
